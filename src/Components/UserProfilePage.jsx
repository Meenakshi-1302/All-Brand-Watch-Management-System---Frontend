    ...
    if (isEditing) {
      try {
        ...
        if (userId) {
          ...
          const response = await axios.put(`http://localhost:8089/customers/${userId}`, formData);

          // Fetch updated user details
          const updatedResponse = await axios.get(`http://localhost:8089/customers/${userId}`);
